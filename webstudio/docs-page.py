...
SECTIONS = [
    ...
    ("copy", "1. Copy the Webstudio project", [
        p("Open the [Webstudio template](https://p-203cb399-37cc-4cff-9d7f-7bd9b24f1c95.apps.webstudio.is/?authToken=8988cf57-da2e-4cec-9320-bc184049b0d4&mode=preview) and select Clone to copy it into your workspace. The copy starts unconnected: pages render, the catalog stays empty and checkout says the shop isn't connected yet, until step 4."),
    ]),
    ("merchant", "2. Deploy Merchant", [
    ...
    ("stripe", "3. Run the setup script", [
        ...
        table(["Key", "Starts with", "Where it goes"],
              ...
              ["Admin key", "`sk_`", "The account Worker (step 6) and your password manager. Never paste it into Webstudio."]),
    ...
    ("account", "6. Optional: customer accounts and order emails", [
        ...
        p("Finally, open the account Worker's URL once in your browser. It registers itself for Merchant's order webhook and answers with its status. Put the URL into `shop.accountUrl` and publish."),
    ...
    ("admin", "7. Optional: admin dashboard", [
        deploy("admin", "Deploy the admin dashboard to Cloudflare"),
        p("Merchant's own dashboard, deployed as a small static Worker. Open its URL and sign in with your Merchant URL and admin key (`sk_…`). The key stays in that browser only. The dashboard manages products, variants, stock, images, orders, customers and discounts."),
    ]),
    ("products", "Managing products", [
        p("Use the [admin dashboard](#admin) (step 7), or the API directly:"),
